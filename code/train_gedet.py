...
flags.DEFINE_integer('epochs', 39, 'Number of epochs to train.')
...
flags.DEFINE_integer('active_epochs', 4, 'Number of active epochs to continue training discriminator.')
...
with open(FLAGS.datasetname + 'gedet++_trainGEDet_test.txt', 'a') as f:
    f.write("\n")
    for attr, flag_obj in FLAGS.__flags.items():
...
adj, features, y_train, y_val, y_test, train_mask, val_mask, test_mask, y_train_active, train_active_mask, ground_truth_train_active, ground_truth_train_active_mask = load_clean_data(
    FLAGS.datapath, FLAGS.datasetname, pca_flag)

## just for running the fig.a
#adj, features, y_train, y_val, y_test, train_mask, val_mask, test_mask= load_clean_data_adjust_ratio(FLAGS.datapath,FLAGS.datasetname)
...
checkpoint_path = FLAGS.datasetname + "training_1/"+'10%error_rate/'
...
def evaluate(features, support, labels, mask, sample_z, placeholders):
    ...
    val_correct, masked_v_pre, masked_v_label, v_discriminator_loss, d_on_active_data, data_embeds = sess.run(
        [model.masked_correct, model.masked_pred_class, model.masked_labels, model.d_loss, model.d_model_labeled_data,
         model.data_features], feed_dict=feed_dict_val)
...
for epoch in range(FLAGS.epochs):
    ...
    print("Epoch",epoch)
    t1e = time.time()
    if 'species' in FLAGS.datasetname and epoch==0:
    ...
    _, _, correct, masked_t_pre, masked_t_label,discriminator_loss, generator_loss, discriminator_unsupervised_loss, discriminator_real_data_loss, discriminator_fake_data_loss =sess.run([model.d_opt, model.g_opt, model.masked_correct,
                                                                  model.masked_pred_class, model.masked_labels,model.d_loss, model.g_loss,model.unsupervised_loss, model.real_data_loss, model.fake_data_loss] , feed_dict=feed_dict)
    num_correct += correct
    ##add the training discriminator loss
    train_d_loss.append(discriminator_loss)
    train_g_loss.append(generator_loss)
    #print("generator loss is {}".format(generator_loss))
    #print("discriminator loss is {}".format(discriminator_loss))
    #print("discriminator unsupervised loss is{}".format(discriminator_unsupervised_loss))
    #print("discriminator fake data loss is{}".format(discriminator_fake_data_loss))
    #print("discriminator real data loss is{}".format(discriminator_real_data_loss))
    #print("discriminator gradients is{}".format(discriminator_gradients))
...
if 'dm' in FLAGS.datasetname:
    update = model.learning_rate.assign(sess.run(model.learning_rate) * (9e-1))
    sess.run(update)

elif 'user17' in FLAGS.datasetname:
    update = model.learning_rate.assign(sess.run(model.learning_rate) * pow((0.9), 20))
    sess.run(update)
