...
flags.DEFINE_string('datasetname', 'user10_error', 'Dataset to be used.')
flags.DEFINE_string('datapath', "dataset/user10/", "Dataset path")
flags.DEFINE_integer('active_epochs',15, 'Number of active epochs to continue training discriminator.')
...
flags.DEFINE_float('pagerank_prob', 0.00, 'probility of going down instead of going back to the starting position in the random walk')
...
# Load data
adj, features, y_train, y_val, y_test, train_mask, val_mask, test_mask, y_train_active, train_active_mask, ground_truth_train_active, ground_truth_train_active_mask = load_clean_data(
    FLAGS.datapath, FLAGS.datasetname, FLAGS.sample_rate, pca_flag)
...
placeholders = {
    ...
    # placehoder for the inputing fake data into discriminator
}
...
def evaluate(features, support, labels, mask, sample_z, placeholders):
    ...
    val_correct, masked_v_pre, masked_v_label, v_discriminator_loss, d_on_active_data, data_embeds, generator_unsupervised_loss = sess.run(
        [model.masked_correct, model.masked_pred_class, model.masked_labels, model.d_loss, model.d_model_labeled_data,
         model.data_features, model.unsupervised_loss], feed_dict=feed_dict_val)
    ...
    return val_accuracy, v_precision[-1], v_recall[-1], v_f1_score[-1], v_discriminator_loss, (
                time.time() - t_test), d_on_active_data, data_embeds
...
if 'dm' in FLAGS.datasetname:
    ...
    update = model.learning_rate.assign(sess.run(model.learning_rate) * pow((0.9), 10))
    sess.run(update)
...
for epoch in range(FLAGS.active_epochs):
    ...
    acc_active_train, pre_active_train, recall_active_train, f1_active_train, discriminator_loss_active_train, duration_active_train, dis_active_data, data_embeds = evaluate(
        features, support, y_train_active, train_active_mask, sample_z, placeholders)
    # print(acc_active_train)
    # print(f1_active_train)
    ...
    if sampling_method == 'random':
        ...
        '''
        sampler_id, train_active_mask = random_sampler(FLAGS.sample_size, train_active_mask, seed,
                                                       ground_truth_train_active)
    elif sampling_method == 'entropy':
        active_entropy_score = entropy_score(dis_active_data)
        sampler_id, train_active_mask = entropy_sampler(FLAGS.sample_size, train_active_mask, active_entropy_score,
                                                        ground_truth_train_active)
        ...
        active_data_embeds = data_embeds[train_active_mask]
        #print(sum(train_active_mask))
        #pre-compute Personalized PageRank matrix
        pr_prob = 1 - FLAGS.pagerank_prob
        page_rank_matrix = cmpt_page_rank(pr_prob, adj)
        ##compute soft-labels for all nodes
        observed_y_train = y_train.copy()
        for id in range(len(train_mask)):
            if train_mask[id]:
                observed_y_train[id] = observed_y_train[id]
            else:
                observed_y_train[id] = [0,0]

        soft_label_matrix = page_rank_matrix.dot(observed_y_train)
        dense_page_rank = page_rank_matrix.toarray()
        ...
        for iter_c in range(num_classes):
            iter_Pi = page_rank_matrix[train_active_mask[iter_c]]
            #iter_gpr = mean(iter_Pi, dim=0).squeeze()
            #gpr_matrix.append(iter_gpr)
        '''


        sampler_id, train_active_mask = soft_approximation_sampler(FLAGS.cluster_size, FLAGS.sample_size, train_active_mask,
                                                              active_data_embeds, ground_truth_train_active, lam,  dense_page_rank,soft_label_matrix, dis_active_data)

    elif sampling_method == 'un_approximation':
        lam = FLAGS.lamda
        active_data_embeds = data_embeds[train_active_mask]
        print(sum(train_active_mask))
        sampler_id, train_active_mask = un_approximation_sampler(FLAGS.cluster_size, FLAGS.sample_size,
                                                                 train_active_mask,
    ...
    num_correct = 0
    print("confirm y_train not modified")
    print(np.sum(np.count_nonzero(y_train, axis=1)))
    # Construct feed dictionary for the training part
    feed_dict = construct_feed_dict(features, support, y_train, train_mask, placeholders)
    feed_dict.update({placeholders['dropout']: FLAGS.dropout})
    feed_dict.update({placeholders['inputs_z']: sample_z})
...
new_saver = tf.train.import_meta_graph(
    './' + checkpoint_path + sampling_method + 'my_test_model-' + str(best_epoch) + '.meta')
